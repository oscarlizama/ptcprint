<?php 
	...
	if($_POST['sql'] == 7){
		$buscar = $_POST['buscar'];
		$i = 0;
		$sql = "SELECT id_producto,nombre_producto, TP.nombre_tipo_producto FROM productos P, tipos_producto TP WHERE estado_producto = ? AND P.id_tipo_producto = TP.id_tipo_producto AND nombre_producto LIKE ?";
		$b = "%".$buscar."%";
		$stmt = $con->prepare($sql);
