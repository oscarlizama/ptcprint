<?php 
	...
	if (validarFecha($fecha_i) && validarFecha($fecha_f)) {
		$sql = "SELECT p.nombre_producto,md.medida,c.fecha_solicitud FROM productos p, carritos c, medidas_producto md WHERE c.id_cliente=? AND p.id_producto = md.id_producto AND c.id_medida = md.id_medida AND c.fecha_solicitud BETWEEN '".$fecha_i."' AND '".$fecha_f."' AND c.recogido=1";
		$stmt = $con->prepare($sql);
		$stmt->execute(array($id_cl));
		while ($datos = $stmt->fetch(PDO::FETCH_BOTH)) {
			$pdf->Cell(70,10,utf8_decode($datos[0]),1,0,'L',0);
		   	$pdf->Cell(35,10,utf8_decode($datos[1]),1,0,'C',0);
		...
		$sql = "SELECT a.nombre_archivo, p.fecha_pedido FROM archivos a, pedidos p, clientes cl, conversaciones c WHERE a.id_archivo = p.id_archivo AND cl.id_cliente=? AND p.fecha_pedido BETWEEN '". $fecha_i ."' AND '".$fecha_f ."' AND cl.id_cliente = c.id_cliente AND c.id_conversacion = a.id_conversacion AND p.recogido=1";
		$stmt = $con->prepare($sql);
		$stmt->execute(array($id_cl));
		while ($datos = $stmt->fetch(PDO::FETCH_BOTH)) {
			$pdf->Cell(70,10,utf8_decode($datos[0]),1,0,'L',0);
			...
			//Muestro la iamgen dentro de la celda GetX y GetY dan las coordenadas actuales de la fila
		 	...
		 	$pdf->Cell(40,10,utf8_decode($datos[1]), 1, 0, 'C', false );
			$pdf->Ln(10);
		}	
	}
	$pdf->SetFont('Arial','',12);
	$pdf->SetTitle("Historial de compras | Punto Print - Soluciones en impresiones" , true);
