<?php 
	...
	while ($data = $stmt->fetch(PDO::FETCH_BOTH)) {
		...
		$sql2 = "SELECT p.nombre_producto, m.medida, COUNT(c.id_medida) FROM productos p, medidas_producto m, carritos c WHERE p.id_producto = m.id_producto AND c.id_medida = m.id_medida GROUP BY c.id_medida";
